...
class CalendarAdapter (private val listener: (calendarDateModel: CalendarDateModel, position: Int) -> Unit):
    ...
    interface onItemClickListener{
        fun onStartItemClick(list: CalendarDateModel)
    }

    private var mListener: onItemClickListener? = null

    fun setOnStartItemClickListener(listener: onItemClickListener){
        mListener = listener
    }
    ...
    override fun onBindViewHolder(holder: CalendarViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            ...
            mListener?.onStartItemClick(itemList)
        }
        if (position == adapterPosition){
            //Seçili Günün Tasarımını Güncelleme
